...
fn view(&self, ctx: &Context<Self>) -> Html {
    ...
    html! {
        <div class="flex w-screen">
            <div class="flex-none w-56 h-screen bg-blue-50">
                <div class="text-xl p-3 font-semibold text-blue-800">{"👥 Active Users"}</div>
                {
                    self.users.clone().iter().map(|u| {
                        html!{
                            <div class="flex m-3 bg-white rounded-lg p-2 hover:bg-blue-100 transition-all cursor-pointer">
                ...
                <div class="w-full grow overflow-auto px-6 py-4 space-y-4">
                    {
                        self.messages.iter().map(|m| {
                            ...
                            let is_self = user.map(|u| u.name.clone()) == Some(ctx.link().context::<User>(Callback::noop()).unwrap().0.username.borrow().clone());

                            let bubble_class = if is_self {
                                "ml-auto bg-blue-100 text-right rounded-tl-lg rounded-bl-lg rounded-br-lg"
                            } else {
